...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $order['pending'] = Order::where('status',0)->count();
        $order['delivered'] = Order::where('status',1)->count();
        $order['processing'] = Order::where('status',2)->count();
        $order['cancel'] = Order::where('status',3)->count();

        $otherData['category'] = Category::count();
        $otherData['product'] = Product::count();
        $otherData['brand'] = Brand::count();

        return view('admin.dashboard', compact('pageTitle', 'widget', 'report', 'chart','payment','depositsMonth','months','deposits','order','otherData'));
    }
